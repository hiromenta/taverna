    ...
    isFeatureActive(featureName: string, role?: Role): Observable<{ feature: string, active: boolean, reason?: string }> {
        return this._configService.getFeaturesConfig().pipe(
            switchMap((features) => {
                ...
                if (feature.grantNone || (feature.grant?.length && !feature.grant?.includes(role || (this._userService.user?.role as Role) || 0))) {
                    return of(this._buildResponse(featureName, false, `Role not granted (Needs: ${feature.grant?.join(', ')} - Selected: ${role || (this._userService.user?.role as Role) || 0})`));
                }

                if (feature.grantAll || feature.grant?.includes(role || (this._userService.user?.role as Role) || 0)) {
